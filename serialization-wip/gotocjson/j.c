        ...
        json_object_foreach(jsonProperties, key, jsonProp) {
          ...
          switch (valueType) {
            ...
            case TimeType:
              ...
              resource->properties.items[i].value.timeValue = json_integer_value(jsonPropValue);
              break;
...
char *encodeMonitoredResource(const MonitoredResource *resource, size_t flags) {
  ...
  if (resource->lastCheckTime) {
    /* TODO: millis number */
    json_object_set_new(json, "lastCheckTime", json_integer(resource->lastCheckTime));
  }
  if (resource->nextCheckTime) {
    /* TODO: millis number */
    json_object_set_new(json, "nextCheckTime", json_integer(resource->nextCheckTime));
  }
  if (resource->lastPlugInOutput) {
    json_object_set_new(json, "lastPlugInOutput", json_string(resource->lastPlugInOutput));
  }
  if (resource->category) {
    json_object_set_new(json, "category", json_string(resource->category));
  }
  if (resource->description) {
    json_object_set_new(json, "description", json_string(resource->description));
  }
  if (resource->properties.count) {  // go:map[string]TypedValue
    ...
    for (size_t i = 0; i < resource->properties.count; i++) {
      ...
      json_object_set_new(jsonProp, "valueType", json_integer(resource->properties.items[i].value.valueType));

      switch (resource->properties.items[i].value.valueType) {
        case IntegerType:
          json_object_set_new(jsonProp, "integerValue", json_integer(resource->properties.items[i].value.integerValue));
          break;

        case DoubleType:
          json_object_set_new(jsonProp, "doubleValue", json_real(resource->properties.items[i].value.doubleValue));
          break;

        case StringType:
          json_object_set_new(jsonProp, "stringValue", json_string(resource->properties.items[i].value.stringValue));
          break;

        case BooleanType:
          json_object_set_new(jsonProp, "boolValue", json_boolean(resource->properties.items[i].value.boolValue));
          break;

        case TimeType:
          /* TODO: millis number */
          json_object_set_new(jsonProp, "timeValue", json_integer(resource->properties.items[i].value.timeValue));
          break;
      ...
      json_object_set_new(jsonProperties, resource->properties.items[i].key, json_deep_copy(jsonProp));
    }
  }
