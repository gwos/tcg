...
func (service *AgentService) CheckLicences(hostsToSendCount int) {
	log.Info("Checking licenses...")

	for _, gwClient := range service.gwClients {
		hostName := "unknown"
		if gwClient.GWConnection != nil {
			hostName = gwClient.GWConnection.HostName
		}

		err := gwClient.Connect()
		if err != nil {
			log.Error("Failed to check license. Unable to establish connection to ", hostName, ": ", err)
			// if failed to get fresh license remove it from cache - it won't send inventory in this case
			cache.LicenseChecksCache.Delete(hostName)
			continue
		}

		// count hosts to allocate
		// for this:
		// get count of hosts sent last time from cache, and count hosts to allocate = diff between curr and last counts
		// if lastSentHostsCount for connection does not exists in cache:
		// count hosts to which services of curr agent are already assigned in GWOS (this means that connector was running
		// and sending inventory before, probably was restarted), use this count of hosts as "last sent"
		...
		if lastSentHostsCountCache, exists := cache.LastSentHostsCountCache.Get(hostName); exists {
			...
			gwServices, err := gwClient.GetServicesByAgent(service.AgentID)
			...
			lastSentHostsCount = len(hostNames)
		}
		hostsToAllocate := hostsToSendCount - lastSentHostsCount
		if hostsToAllocate < 0 {
			hostsToAllocate = 0
		}

		// request license check
		license, err := gwClient.CheckLicense(hostsToAllocate)
		if err != nil || license == nil {
			log.Error("Failed to check license on ", hostName)
			if err != nil {
				log.Error(err)
			} else {
				log.Error("Response is nil")
		...
		cache.LicenseChecksCache.SetDefault(hostName, license)

		// if license is ok to send inventory update LastSentHostsCountCache with curr hostsToSendCount value
		// keep prev value for a case if send fails and curr inventory won't be send
		// to revert LastSentHostsCount to this prev value then
		if license.IsOkToSendInventory() {
			cache.PrevSentHostsCountCache.SetDefault(hostName, lastSentHostsCount)
			cache.LastSentHostsCountCache.SetDefault(hostName, hostsToSendCount)
		}
	}
}

func (service *AgentService) RevertLastSentHostsCountCache() {
	log.Info("Reverting last sent hosts count cache...")
	for _, gwClient := range service.gwClients {
		if gwClient.GWConnection != nil {
			hostName := gwClient.GWConnection.HostName
			if prevSentHostsCount, exists := cache.PrevSentHostsCountCache.Get(hostName); exists {
				cache.LastSentHostsCountCache.SetDefault(hostName, prevSentHostsCount)
			} else {
				cache.LastSentHostsCountCache.Delete(hostName)
			}
		}
	}
}
