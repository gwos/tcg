...
func process_parse_nodes(
	...
	for _, s := range f.Imports {
		...
		// That said, references to the "logzer" package we supply do not need any such special handling,
		// because we won't actually be creating conversion routines for any datatypes in that package.
		// The special-case handling here of that package prevents a #include "logzer.h" directive from
		...
		if special_package != nil && special_package[1] != "logzer" {
